...
Przykład, kiedy przyda się leniwa ewaluacja - pisząc funkcję, która generuje ciąg możliwych rozwiązań problemu (np. wypełnione plansze do sudoku) nie musimy przejmować się, żeby w tej samej funkcji sprawdzać poprawność wypełnieni. Możemy zrobić to w osobnej funkcji, a na koniec stwierdzić tylko "chcę pierwsze, co wyjdzie".
```haskell
head $ filter correct_solution $ generate_values data
-- albo: head (filter correct_solution (generate_values data))
```

Inny przykład - separacja IO od obliczeń. Zamiast samemutroszczyć się o to, by jak najszybciej wyświetlać wygenerowane modele, tworzymy funkcję, które je konstruuje, a potem mówimy "wyświetl wszystko". Leniwa ewaluacja zapewni nam, że modele będą tworzone dopiero wtedy, kiedy będą faktycznie potrzebne (nie będzie wielkiej pauzy napoczątku), a każdy będzie wyświetlony jak tylko będzie gotowy.
```haskell
